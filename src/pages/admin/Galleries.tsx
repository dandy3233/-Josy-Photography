...
import { useToast } from '@/hooks/use-toast';
import { uploadPhoto, deletePhoto } from '@/lib/storage';
import { Plus, Pencil, Trash2, Image, Loader2, Eye, EyeOff, Star, Images, Tag } from 'lucide-react';
import {
  Select,
  SelectContent,
  SelectItem,
...
const GalleriesPage = () => {
  ...
  const closeDialog = () => {
    setDialogOpen(false);
    setEditingGallery(null);
    setFormData({ title: '', description: '', category: '', is_public: true, is_featured: false });
    setCoverImage(null);
  };
                ...
                <CardContent className="p-4">
                  ...
                  {gallery.category && (
                    <div className="flex items-center gap-1 mt-1">
                      ...
                      <span className="text-xs text-primary font-body">{gallery.category}</span>
                    </div>
                  )}
                  {gallery.description && (
                    <p className="text-sm text-muted-foreground mt-1 line-clamp-2">{gallery.description}</p>
